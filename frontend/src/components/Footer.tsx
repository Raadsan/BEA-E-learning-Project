...
export default function Footer() {
  ...
  const socialLinks = contactPage?.social_links || {};

  const socialLinkProps = (network: string, label: string) => {
    const href = socialLinks[network]?.trim();

    return {
      href: href || "#",
      target: href ? "_blank" : undefined,
      rel: href ? "noopener noreferrer" : undefined,
      onClick: href ? undefined : handleComingSoon,
      "aria-label": label,
    };
  };
  ...
  const handleComingSoon = (event: MouseEvent<HTMLAnchorElement>) => {
    ...
    setComingSoonItem(event.currentTarget.getAttribute('aria-label') || event.currentTarget.textContent?.trim() || 'This feature');
  };
  ...
  return (
    <footer className="text-white py-6 sm:py-8 lg:py-10" style={{ backgroundColor: isDarkMode ? '#03002e' : '#010080' }}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
          ...
          <div className="flex flex-col items-start sm:items-start">
            ...
            <div className="mt-4 sm:mt-auto sm:ml-5 text-left sm:text-left">
              ...
              <div className="flex gap-2 justify-start sm:justify-start">
                {/* Facebook */}
                <a {...socialLinkProps("facebook", "Facebook")} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-transparent border border-white hover:bg-white/20' : 'bg-white hover:bg-gray-200'}`}>
                ...
                {/* Instagram */}
                <a {...socialLinkProps("instagram", "Instagram")} className={`w-8 h-8 rounded-md flex items-center justify-center transition-colors ${isDarkMode ? 'bg-transparent border border-white hover:bg-white/20' : 'bg-white hover:bg-gray-200'}`}>
                  <svg className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-[#010080]'}`} fill="currentColor" viewBox="0 0 24 24">
                ...
                {/* Twitter/X */}
                <a {...socialLinkProps("twitter", "X / Twitter")} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-transparent border border-white hover:bg-white/20' : 'bg-white hover:bg-gray-200'}`}>
                  <svg className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-[#010080]'}`} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
                  </svg>
                </a>
                {/* YouTube */}
                <a {...socialLinkProps("youtube", "YouTube")} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-transparent border border-white hover:bg-white/20' : 'bg-white hover:bg-gray-200'}`}>
                  <svg className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-[#010080]'}`} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                  </svg>
                </a>
                {/* LinkedIn */}
                <a {...socialLinkProps("linkedin", "LinkedIn")} className={`w-8 h-8 rounded-md flex items-center justify-center transition-colors ${isDarkMode ? 'bg-transparent border border-white hover:bg-white/20' : 'bg-white hover:bg-gray-200'}`}>
                  <svg className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-[#010080]'}`} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                  </svg>
                </a>
                {/* Telegram */}
                <a {...socialLinkProps("telegram", "Telegram")} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-transparent border border-white hover:bg-white/20' : 'bg-white hover:bg-gray-200'}`}>
                  <svg className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-[#010080]'}`} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
                  </svg>
                </a>
                {/* TikTok */}
                <a {...socialLinkProps("tiktok", "TikTok")} className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-transparent border border-white hover:bg-white/20' : 'bg-white hover:bg-gray-200'}`}>
                  <svg className={`w-5 h-5 ${isDarkMode ? 'text-white' : 'text-[#010080]'}`} fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="text-left sm:text-left">
            ...
            <ul className="space-y-2 text-sm text-white" style={{ fontFamily: 'var(--font-playfair)' }}>
              ...
              <li><Link href="/auth/login" className="hover:text-gray-300 transition-colors">Student E-Learning Portal</Link></li>
          ...
          <div className="text-left sm:text-left">
            ...
            <ul className="space-y-2 text-sm text-white" style={{ fontFamily: 'var(--font-playfair)' }}>
              <li><a href="#" onClick={handleComingSoon} className="hover:text-gray-300 transition-colors">The BEA Handbook</a></li>
              <li><a href="#" onClick={handleComingSoon} className="hover:text-gray-300 transition-colors">BEA Programs Portfolio</a></li>
              <li><a href="#" onClick={handleComingSoon} className="hover:text-gray-300 transition-colors">IELTS and TOEFL Test Guide</a></li>
              <li><a href="#" onClick={handleComingSoon} className="hover:text-gray-300 transition-colors">A-Z Academic Writing Program Guide</a></li>
              <li><a href="#" onClick={handleComingSoon} className="hover:text-gray-300 transition-colors">ESP Program Guide</a></li>
            </ul>
          </div>
        </div>
        ...
        <div className="border-t border-white/20 pt-4 sm:pt-6 mt-6 sm:mt-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-4 text-xs sm:text-sm text-white" style={{ fontFamily: 'var(--font-playfair)' }}>
          ...
          <div className="flex flex-wrap justify-start sm:justify-center items-center gap-3 sm:gap-4">
            ...
            <Link href="/website/terms-and-conditions" className="hover:text-gray-300 transition-colors">Terms & Conditions</Link>
          </div>
        </div>
      </div>
      ...
      {comingSoonItem && (
        ...
        >
          <div
            className="relative w-full max-w-md overflow-hidden rounded-2xl bg-white p-8 text-center text-gray-700 shadow-2xl sm:p-10"
            onClick={(event) => event.stopPropagation()}
          >
            <button
              type="button"
              onClick={() => setComingSoonItem(null)}
              className="absolute right-4 top-3 text-2xl text-gray-400 transition-colors hover:text-[#010080]"
              aria-label="Close coming soon message"
            >
              ×
            </button>
            <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-blue-50 text-[#010080]">
            ...
            <p className="mb-2 text-xs font-bold uppercase tracking-[0.25em] text-blue-600">{comingSoonItem}</p>
            <h2 id="coming-soon-title" className="font-serif text-3xl font-bold text-[#010080]">Coming Soon</h2>
            <p className="mx-auto mt-3 max-w-sm text-sm leading-6 text-gray-600">
              We&apos;re preparing this feature for you. Please check back soon for the latest update.
            </p>
            <button
              type="button"
              onClick={() => setComingSoonItem(null)}
              className="mt-7 w-full rounded-lg bg-[#010080] px-5 py-3 font-semibold text-white transition-colors hover:bg-blue-900"
            >
              Got it
            </button>
          </div>
