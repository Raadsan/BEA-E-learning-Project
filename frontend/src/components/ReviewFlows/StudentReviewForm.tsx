...
import { useSubmitTeacherReviewMutation, useGetQuestionsQuery, useGetActiveReviewAssignmentQuery } from "@/lib/api/reviewApi";
...
const StudentReviewForm = ({ teacher, classId, termSerial, onComplete }) => {
    ...
    const { data: activeAssignment, isLoading: assignmentLoading } = useGetActiveReviewAssignmentQuery({ type: "teacher", class_id: classId }, { skip: !classId });
    const { data: fallbackQuestions = [], isLoading: questionsLoading } = useGetQuestionsQuery("teacher");
    const questions = activeAssignment?.questions?.length ? activeAssignment.questions : fallbackQuestions;

    const handleOpen = () => {
        if (!questions.length) {
            toast.error("Su'aalaha qiimeynta wali lama diyaarin.");
            return;
        }
        setIsModalOpen(true);
    };
    ...
    return (
        <>
            <button
                onClick={handleOpen}
                disabled={assignmentLoading || questionsLoading}
            ...
            >
                {assignmentLoading || questionsLoading ? (
                    ...
                    <>
                        <svg className="w-5 h-5 transition-transform group-hover:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
            ...
            <ReviewModal
                ...
                title={activeAssignment?.title || "Qiimeynta Baraha"}
                subtitle={activeAssignment?.description || "Fadlan qiimeyn dhab ah ka bixi barahaaga"}
