...
export default function ContactUs() {
  ...
  useEffect(() => {
    ...
    const loadPageContent = () => {
      fetch(`${API_URL}/contact/page-content`, { cache: "no-store" })
        .then((response) => response.ok ? response.json() : Promise.reject(new Error("Failed to load contact content")))
        .then((data) => { if (isMounted) setPageContent(data); })
        .catch((error) => console.error(error));
    };

    loadPageContent();
    const refreshInterval = window.setInterval(loadPageContent, 3000);
    window.addEventListener("focus", loadPageContent);

    return () => {
      isMounted = false;
      window.clearInterval(refreshInterval);
      window.removeEventListener("focus", loadPageContent);
    };
  }, []);
      ...
      if (response.ok && data.success) {
        showToast(
          data.emailSent
            ...
            : "Message saved successfully! We will contact you soon.",
          "success"
        );
        // Reset form
      ...
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className={`text-4xl sm:text-5xl lg:text-6xl font-bold text-white mb-4 ${visibleSections.hero ? 'animate-fade-in-down' : 'opacity-0'}`}>
            {pageContent?.hero_title || "Get In Touch With Us"}
          </h1>
          <p className={`text-2xl sm:text-3xl font-bold text-white mb-6 ${visibleSections.hero ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
            {pageContent?.hero_subtitle || "We're here to help you 24/7"}
          </p>
          <p className={`text-white text-base sm:text-lg max-w-3xl mx-auto ${visibleSections.hero ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '0.3s' }}>
            {pageContent?.hero_description || "Have questions about our courses, need support, or want to discuss your learning goals? Our team is ready to assist you on your educational journey."}
          </p>
        </div>
      </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className={`flex items-start gap-4 ${visibleSections.contact ? 'animate-fade-in-left' : 'opacity-0'}`} style={{ animationDelay: '0.1s' }}>
                  ...
                  <div>
                    ...
                    <p className={isDarkMode ? 'text-white' : 'text-gray-700'}>{pageContent?.address || "HQ: Taleex, Hodan District, Mogadishu, Somalia"}</p>
                  </div>
                </div>
                ...
                <div className={`flex items-start gap-4 ${visibleSections.contact ? 'animate-fade-in-right' : 'opacity-0'}`} style={{ animationDelay: '0.2s' }}>
                  ...
                  <div>
                    ...
                    <p className={isDarkMode ? 'text-white' : 'text-gray-700'}>{pageContent?.phone || "+252 61 123-4567"}</p>
                  </div>
                </div>
                ...
                <div className={`flex items-start gap-4 ${visibleSections.contact ? 'animate-fade-in-left' : 'opacity-0'}`} style={{ animationDelay: '0.3s' }}>
                  ...
                  <div>
                    ...
                    <p className={isDarkMode ? 'text-white' : 'text-gray-700'}>{pageContent?.email || "admission@beaportal.com"}</p>
                  </div>
                </div>
                ...
                <div className={`flex items-start gap-4 ${visibleSections.contact ? 'animate-fade-in-right' : 'opacity-0'}`} style={{ animationDelay: '0.4s' }}>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {/* Facebook */}
                      <a href={pageContent?.social_links?.facebook || "#"} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-white header-keep-white hover:bg-gray-100' : 'bg-[#010080] hover:bg-[#010060]'}`}>
                      ...
                      {/* Instagram */}
                      <a href={pageContent?.social_links?.instagram || "#"} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-md flex items-center justify-center transition-colors ${isDarkMode ? 'bg-white header-keep-white hover:bg-gray-100' : 'bg-[#010080] hover:bg-[#010060]'}`}>
                        <svg className={`w-5 h-5 ${isDarkMode ? 'text-[#010080]' : 'text-white'}`} fill="currentColor" viewBox="0 0 24 24">
                      ...
                      {/* Twitter/X */}
                      <a href={pageContent?.social_links?.twitter || "#"} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-white header-keep-white hover:bg-gray-100' : 'bg-[#010080] hover:bg-[#010060]'}`}>
                        <svg className={`w-5 h-5 ${isDarkMode ? 'text-[#010080]' : 'text-white'}`} fill="currentColor" viewBox="0 0 24 24">
                          <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
                        </svg>
                      </a>
                      {/* YouTube */}
                      <a href={pageContent?.social_links?.youtube || "#"} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-white header-keep-white hover:bg-gray-100' : 'bg-[#010080] hover:bg-[#010060]'}`}>
                        <svg className={`w-5 h-5 ${isDarkMode ? 'text-[#010080]' : 'text-white'}`} fill="currentColor" viewBox="0 0 24 24">
                          <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
                        </svg>
                      </a>
                      {/* LinkedIn */}
                      <a href={pageContent?.social_links?.linkedin || "#"} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-md flex items-center justify-center transition-colors ${isDarkMode ? 'bg-white header-keep-white hover:bg-gray-100' : 'bg-[#010080] hover:bg-[#010060]'}`}>
                        <svg className={`w-5 h-5 ${isDarkMode ? 'text-[#010080]' : 'text-white'}`} fill="currentColor" viewBox="0 0 24 24">
                          <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                        </svg>
                      </a>
                      {/* Telegram */}
                      <a href={pageContent?.social_links?.telegram || "#"} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-white header-keep-white hover:bg-gray-100' : 'bg-[#010080] hover:bg-[#010060]'}`}>
                        <svg className={`w-5 h-5 ${isDarkMode ? 'text-[#010080]' : 'text-white'}`} fill="currentColor" viewBox="0 0 24 24">
                          <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
                        </svg>
                      </a>
                      {/* TikTok */}
                      <a href={pageContent?.social_links?.tiktok || "#"} target="_blank" rel="noreferrer" className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${isDarkMode ? 'bg-white header-keep-white hover:bg-gray-100' : 'bg-[#010080] hover:bg-[#010060]'}`}>
                        <svg className={`w-5 h-5 ${isDarkMode ? 'text-[#010080]' : 'text-white'}`} fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            <h2 className={`text-3xl sm:text-4xl font-bold mb-4 ${visibleSections.schedule ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ color: isDarkMode ? '#ffffff' : '#010080' }}>
              {pageContent?.schedule_title || "Operational Schedule"}
            </h2>
            <p className={`text-base sm:text-lg mb-8 ${visibleSections.schedule ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: '0.1s', color: isDarkMode ? '#ffffff' : '#010080' }}>
              {pageContent?.schedule_description || "If you were wondering how many days or hours we work, here is our day-to-day operational schedule for your reference."}
            </p>
            ...
            <div className={`rounded-lg shadow-md p-6 sm:p-8 ${visibleSections.schedule ? 'animate-scale-in' : 'opacity-0'} ${isDarkMode ? 'bg-[#050040]' : 'bg-white'}`} style={{ animationDelay: '0.2s' }}>
              <div className="space-y-4">
                {(pageContent?.schedule || []).map((row, index) => (
                  <div key={`${row.day}-${index}`} className={`flex justify-between items-center py-3 ${index < pageContent.schedule.length - 1 ? `border-b ${isDarkMode ? 'border-[#1a1a3e]' : 'border-gray-200'}` : ''}`}>
                    <span className="font-semibold" style={{ color: isDarkMode ? '#ffffff' : '#010080' }}>{row.day}</span>
                    <span style={{ color: isDarkMode ? '#ffffff' : '#010080' }}>{row.hours}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className={`rounded-lg shadow-md p-6 sm:p-8 ${visibleSections.form ? 'animate-fade-in-up' : 'opacity-0'} ${isDarkMode ? 'bg-[#050040]' : 'bg-white'}`} style={{ animationDelay: '0.1s' }}>
              ...
              {/* Status messages are now handled by toasts */}

              <button
                type="submit"
                disabled={isSubmitting}
                className={`w-full px-6 py-4 rounded-lg font-semibold text-lg transition-colors ${isSubmitting
                  ? "opacity-50 cursor-not-allowed"
                  : isDarkMode
                    ? "bg-white header-keep-white text-[#010080] hover:bg-gray-100"
