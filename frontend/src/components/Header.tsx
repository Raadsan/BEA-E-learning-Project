  ...
  return (
    <header className={`sticky top-0 z-50 transition-all duration-300 ${isDarkMode ? 'bg-[#03002e]' : (mobileMenuOpen ? 'bg-white' : 'bg-white/95 backdrop-blur-sm')}`} style={{ fontFamily: 'var(--font-opensans)' }}>
      ...
      <div className="container mx-auto px-3 sm:px-4 md:px-5 lg:px-6 xl:px-8">
        <div className="flex items-center justify-between h-14 sm:h-16 md:h-[4.25rem] lg:h-20 xl:h-24 overflow-visible">
          {/* 1. Logo (Left Column) */}
          <div className="flex items-center relative h-full overflow-visible">
            <Link href="/" className="flex-shrink-0 flex items-center h-full overflow-visible transition-transform hover:scale-105 duration-300 relative z-10">
              <Image
                src={isDarkMode ? "/images/footerlogo-removebg-preview.png" : "/images/headerlogo.png"}
                alt="BEA Logo"
                width={520}
                height={170}
                className="h-10 sm:h-11 md:h-12 lg:h-14 xl:h-16 w-auto object-contain drop-shadow-md scale-[2] sm:scale-[2.15] lg:scale-[2.35] xl:scale-[2.55] origin-[left_center]"
                priority
              />
            </Link>
          </div>

          <nav className="hidden lg:flex items-center justify-start gap-2 lg:gap-3 xl:gap-4 2xl:gap-6 ml-16 lg:ml-32 xl:ml-52 2xl:ml-60 mr-2 lg:mr-3 xl:mr-4 flex-1 font-nav">
          ...
          <div className="flex items-center justify-end gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 xl:gap-5">
            ...
            <Link href="/auth/login" className={`hidden lg:flex items-center justify-center w-24 py-2 rounded-lg font-semibold transition-all duration-300 shadow-sm hover:shadow-md text-sm ${isDarkMode ? '!bg-white !text-black hover:!bg-white/90' : 'bg-[#010080] text-white hover:bg-[#010060]'}`}>
      ...
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 top-[80px] sm:top-[88px] md:top-[92px] lg:top-[104px] xl:top-[120px] bg-black/20 backdrop-blur-sm z-40"
          onClick={() => setMobileMenuOpen(false)}
        />
      )}
      {(programsOpen || beaValuesOpen) && !mobileMenuOpen && (
        <div
          className="fixed inset-0 top-[80px] sm:top-[88px] md:top-[92px] lg:top-[104px] xl:top-[120px] z-40 lg:hidden"
