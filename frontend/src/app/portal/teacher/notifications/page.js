                            ...
                            >
                                <div className="flex items-center p-4 gap-4">
                                    <div className="flex-shrink-0">
                                        <div className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-bold text-lg ${notification.type === 'freezing_request' ? 'bg-amber-500' :
                                            notification.type === 'session_change_request' ? 'bg-blue-600' : 'bg-purple-600'
                                            }`}>
                ...
                <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={closeModal}>
                    ...
                    >
                        <div className={`px-6 py-4 border-b flex items-center justify-between ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
                            <h3 className={`text-xl font-bold ${textMain}`}>Request Details</h3>
                        ...
                        <div className="p-6">
                            <div className="flex items-center gap-4 mb-6">
                                <div className="flex-shrink-0">
                                    <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white font-bold text-2xl ${selectedNotification.type === 'freezing_request' ? 'bg-amber-500' :
                                        selectedNotification.type === 'session_change_request' ? 'bg-blue-600' : 'bg-purple-600'
                                        }`}>
                            ...
                            <div className={`p-4 rounded-lg mb-6 ${isDark ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                                <h5 className={`text-sm font-semibold mb-2 uppercase tracking-wide ${textSub}`}>Details</h5>
                                ...
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    {(() => {
                                        try {
                                            ...
                                            return (
                                                <>
                                                    {metadata.studentName && renderField("Student", metadata.studentName, "student")}
                                                    {metadata.reason && renderField("Reason", metadata.reason, "reason")}
                                                    ...
                                                    {metadata.requestedSession && renderField("Requested Session", metadata.requestedSession, "session")}
                                                </>
                                            );
                                        } catch (e) {
                                            return null;
