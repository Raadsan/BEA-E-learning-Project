...
export default function AdminProficiencyResultDetailsPage() {
    ...
    const { data: allResults, isLoading: resultsLoading } = useGetAllProficiencyResultsQuery();
            ...
            <div className="py-6 px-8 w-full">
                ...
                <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-200 mb-8">
                    <div className="flex flex-col md:flex-row justify-between gap-6 border-b border-gray-100 pb-6 mb-6">
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">{result.full_name || result.student_name}</h1>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <div>
                            ...
                            <p className="text-xl font-bold text-[#010080]">{result.score} <span className="text-sm font-normal text-gray-400">/ {result.total_points || result.total_questions}</span></p>
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Accuracy</p>
                            <p className="text-xl font-bold text-gray-900">{result.percentage ? Math.round(result.percentage) + '%' : '-'}</p>
                        </div>
                        <div>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Submitted</p>
                            <p className="text-sm font-semibold text-gray-700">{new Date(result.submitted_at || result.created_at).toLocaleDateString()}</p>
                        </div>
                        <div>
                        ...
                        <div className="flex gap-3 flex-wrap">
                            {(() => {
                                try {
                                    ...
                                    if (typeof feedback === 'object' && feedback !== null) {
                                        return (
                                            <>
                                                {feedback.essay && (
                                                    <a
                                                        href={resolveMediaUrl(feedback.essay) || "#"}
                                                        target="_blank"
                                                ...
                                                {feedback.audio && (
                                                    <a
                                                        href={resolveMediaUrl(feedback.audio) || "#"}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="flex items-center gap-2 px-4 py-2.5 bg-white text-green-700 border border-green-300 rounded-lg text-sm font-semibold hover:bg-green-50 transition-colors shadow-sm"
                                                    >
                                    ...
                                    return (
                                        <a
                                            href={resolveMediaUrl(result.feedback) || "#"}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="flex items-center gap-2 px-4 py-2.5 bg-white text-green-700 border border-green-300 rounded-lg text-sm font-semibold hover:bg-green-50 transition-colors shadow-sm"
                                        >
                        ...
                        <div key={idx} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
                            ...
                            {(q.type === 'mcq' || q.type === 'multiple_choice') && (
                                <div className="space-y-3">
                                    <p className="text-base font-medium text-gray-900">{q.questionText || q.question}</p>
