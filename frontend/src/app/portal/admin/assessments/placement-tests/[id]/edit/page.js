...
export default function EditPlacementTestPage() {
    ...
    useEffect(() => {
        if (test) {
            ...
            // Ensure questions is an array and assign parts if missing (for legacy data)
            const fetchedQuestions = Array.isArray(test.questions) ? test.questions.map((q, idx) => {
                ...
                if (normalized.type === 'mcq') {
                    ...
                    // If no part, try to guess or default
                    if (!normalized.part) {
                        // Very simple guess: if it's the first few MCQs it's part 1, if it's after many questions it's part 4
                ...
                <div className="w-full px-8 py-6">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pt-20">
                        <div>
                            ...
                            <p className="mt-2 text-gray-600 tracking-tight">Modify questions for each of the 4 parts sequentially.</p>
                        </div>
                    </div>
                        ...
                        <div className="lg:col-span-3 space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            ...
                                            <select
                                                ...
                                                onChange={handleTestChange}
                                                className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#010080]/20 focus:border-[#010080] outline-none transition-colors bg-white cursor-pointer"
                                            >
                            ...
                            <div className="flex items-center justify-between pt-4">
                                ...
                                {currentStep < 4 ? (
                                    <button
                                        onClick={nextStep}
                                        className="flex items-center gap-2 px-8 py-2.5 rounded-lg font-bold text-white bg-[#010080] hover:bg-[#000066] transition-all shadow-md active:scale-95"
                                    >
                                        Next Part
                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <span className="text-sm font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full border border-green-100 animate-pulse">
                                        Ready to Update
                                    </span>
                                )}
                            </div>
                        ...
                        <div className="lg:col-span-1">
                            <div className="sticky top-8 space-y-4 max-h-[calc(100vh-100px)] overflow-y-auto custom-scrollbar pr-2">
                                ...
                                <button
                                    ...
                                    disabled={isUpdating || questions.filter(q => q.part === 4).length === 0}
                                ...
                                >
                                    {isUpdating ? 'Updating Test...' : 'Save All Changes'}
                                </button>
                            </div>
                        </div>
                    </div>
