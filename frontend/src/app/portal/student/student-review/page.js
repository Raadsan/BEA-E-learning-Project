    ...
    <div className={`min-h-screen transition-colors ${bg}`}>
      <div className="p-6">
        ...
        <div className={`mb-6 p-6 rounded-xl shadow ${card}`}>
          <h1 className="text-2xl font-bold mb-2">Student Review</h1>
          <p className={isDark ? "text-gray-300" : "text-gray-600"}>
            View feedback and reviews from your teachers and instructors.
          </p>
        </div>
        ...
        {reviews.length === 0 ? (
          ...
          <div className="space-y-4">
            {reviews.map((review) => (
              <div key={review.id} className={`p-6 rounded-xl shadow ${card}`}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className={`text-lg font-semibold ${isDark ? "text-white" : "text-gray-900"}`}>
                      {review.teacherName || "Teacher"}
                    </h3>
                ...
                <p className={isDark ? "text-gray-300" : "text-gray-700"}>{review.feedback}</p>
                {review.rating && (
