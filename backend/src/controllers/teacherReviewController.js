...
const BOX_KIND = 'teacher_review_box';
...
const findOpenAssignment = async ({ assignmentId, classId, courseId, subprogramId, programId }) => {
    ...
    if (id) {
        const row = await prisma.teacher_review_questions.findUnique({ where: { id } });
    ...
    const rows = await prisma.teacher_review_questions.findMany({ where: { is_active: true }, orderBy: { created_at: 'desc' } });
...
const populateReview = async (review) => {
    ...
    if (review.assignment_id) {
        const found = await prisma.teacher_review_questions.findUnique({ where: { id: review.assignment_id } });
...
export const submitTeacherReview = async (req, res) => {
    try {
        const { teacher_id, class_id, term_serial, rating, comment, answers, assignment_id } = req.body;
...
export const getTeacherReviews = async (req, res) => {
    try {
        ...
        const reviews = await prisma.teacher_reviews.findMany({ where: { teacher_id: String(teacher_id) }, orderBy: { created_at: 'desc' } });
...
export const getAllTeacherReviews = async (req, res) => {
    try {
        ...
        const reviews = await prisma.teacher_reviews.findMany({ where, orderBy: { created_at: 'desc' } });
        res.json(await Promise.all(reviews.map(populateReview)));
    } catch (err) { res.status(500).json({ error: err.message }); }
};

export const getActiveTeacherReviewAssignment = async (req, res) => {
    try {
...
export const getTeacherReviewAssignments = async (_req, res) => {
    try {
        const rows = await prisma.teacher_review_questions.findMany({ orderBy: { created_at: 'desc' } });
...
export const createTeacherReviewAssignment = async (req, res) => {
    try {
        ...
        const row = await prisma.teacher_review_questions.create({
            data: {
...
export const updateTeacherReviewAssignment = async (req, res) => {
    try {
        const id = parseInt(req.params.id, 10);
        const existingRow = await prisma.teacher_review_questions.findUnique({ where: { id } });
        const existing = parseAssignmentRow(existingRow);
        if (!existing) return res.status(404).json({ error: 'Review box not found' });

        const row = await prisma.teacher_review_questions.update({
            where: { id },
            data: {
...
export const deleteTeacherReviewAssignment = async (req, res) => {
    try {
        await prisma.teacher_review_questions.delete({ where: { id: parseInt(req.params.id, 10) } });
...
export const getQuestions = async (req, res) => {
    try {
        const rows = await prisma.teacher_review_questions.findMany({ where: { is_active: true } });
...
export const updateQuestion = async (req, res) => {
    try {
        const updated = await prisma.teacher_review_questions.update({ where: { id: parseInt(req.params.id, 10) }, data: req.body });
        res.json(updated);
    } catch (err) { res.status(500).json({ error: err.message }); }
};
